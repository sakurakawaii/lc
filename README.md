...
* **Zip Extraction Security:** The system currently utilizes Python's standard `zipfile.extractall()` for prototyping speed. In a hardened production environment, this must be replaced with manual path sanitization to prevent Zip Slip directory traversal attacks.
