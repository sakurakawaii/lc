...
*   **LLM Engine (Logic & OCR):**
    *   `.png` / Scanned `.pdf`: Bypasses local text parsing; sent directly to Claude 3.5 Sonnet Vision API for robust OCR and understanding.
    *   **Core Logic:** All locally extracted text is fed to Claude 3.5 Sonnet to determine relevance and return strict JSON categorization (Stage 1).
*   **Two-Pass Hybrid Sanitization Pipeline:**
    ...
    2.  **Semantic Scrubbing (LLM):** The pre-scrubbed text is passed to Claude 3.5 Sonnet to catch residual contextual PII (e.g., coworker names, implied locations) that bypassed the regex filters.
